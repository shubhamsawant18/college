...
function BBA() {
  ...
  React.useEffect(() => {
    const handleClickOutside = (event) => {
      if (!event.target.closest(".bba_dm_selected-filters")) {
  ...
  const renderFilterDropdowns = () => {
    return Object.entries(filterOptions).map(([filterType, options]) => (
      <div key={filterType} className="bba_dm_filter-dropdown-container">
        <button
          className={`bba_dm_dropdown-button ${
        ...
        {activeDropdown === filterType && (
          <div className="bba_dm_filter-dropdown-menu">
            {options.map((option) => (
              <div
                key={option}
                className={`bba_dm_filter-option ${
  ...
  return (
    <>
    <Navbar />
    <div className='bba_dm_bba-container'>
    {/* Heading */}
      <div className='bba_dm_breadcrumb'>

      <a href='/' className='bba_dm_home-icon'>
      <AiFillHome />
      </a>
    ...
    <div className="bba_dm_read-more-container">
      {/* Profile Section */}
      <div className="bba_dm_profile-section">
        <img src="/assets/bba_images/bba-profile.jpg" alt="profile icon" className="bba_dm_profile-icon" />
        <div className="bba_dm_profile-info">
          <p className="bba_dm_profile-name">
            John Doe <span className="bba_dm_checkmark">✔</span>
          </p>
          <span className="bba_dm_designation">Senior Content Specialist</span>
        </div>
      </div>
      ...
      <p>
        {isExpanded ? (
          <>
            ...
            {/* Table of Content */}
            <div className="bba_dm_table-of-content">
            ...
            <div className='bba_dm_Top_10_bba'>
              ...
              <p>The table below discusses in detail the top 10 BBA colleges in India, along with the total number
                ...
                <div className="bba_dm_college-table-container">
                <table className="bba_dm_all-college-table">
                  <thead>
                    <tr>
                      <th className="bba_dm_collegename">College Name</th>
                      <th>Total Seats</th>
            ...
            <div className="bba_dm_placement-wise-clg bba_dm_Top_10_bba">
            ...
            <div className="bba_dm_Top_10_bba">
              ...
              <p>Tabulated below are the top BBA colleges as per India Today rankings across different years.</p>
                ...
                <table className="bba_dm_all-college-table">
                  <thead>
                    <tr>
                      <th className="bba_dm_collegename">Name of the College</th>
            ...
            <div className="bba_dm_Addmission-process  bba_dm_Top_10_bba">
      ...
      <button onClick={toggleContent} className='bba_dm_readmore-btn bba_dm_rmbtn'>
        {isExpanded ? "Read Less" : "Read More"}
      </button>
    </div>


    <div className="bba_dm_filter-bar bba_dm_sticky-filter-bar">
          <button className="bba_dm_filter-button" onClick={clearFilters}>
        ...
        {Object.keys(filters).length > 0 && (
          <div className="bba_dm_active-filters">
            {Object.entries(filters).map(
              ([type, value]) =>
                value && (
                  <span key={type} className="bba_dm_filter-tag">
                    {type.replace(/([A-Z])/g, " $1").trim()}: {value}
                    <button
                      className="bba_dm_remove-filter"
      ...
      {/* College table*/}
      <div className="bba_dm_found-colleges">
          <span>Found {colleges.length} Colleges</span>
        <div className="bba_dm_sorting">
            <label>Sort by:</label>
            <div className="bba_dm_radio-group">
              <label>
                <input
                  type="radio"
                  name="sort"
      ...
      <React.Fragment key={tableIndex}>
            <table className="bba_dm_college-table" key={tableIndex}>
              <thead>
                <tr>
                  <th>CD Rating</th>
                  <th>Colleges</th>
              ...
              <tbody>
                {chunk.map((college, index) => (
                  <tr key={index}>
                    <td className="bba_dm_center">{college.rating}</td>
                    <div className="bba_dm_college-info">
                      <img
                        src={college.logo}
                        alt={`${college.name} logo`}
                        className="bba_dm_college-logo"
                      />
                      <div>
                        <a href={college.link} className="bba_dm_college-name">
                          {college.name}
                        </a>
                        <p className="bba_dm_college-location">{college.location}</p>
                      </div>
                    </div>
                    <div className="bba_dm_college-actions">
                      <a href={college.applyLink} className="bba_dm_action-link bba_dm_apply">
                        &rarr; Apply Now
                      </a>
                      <a
                        href={college.brochureLink}
                        className="bba_dm_action-link bba_dm_brochure"
                      >
                        &#x1F4E5; Download Brochure
                      </a>
                      <label>
                        <input type="checkbox" className="bba_dm_compare-checkbox" />
                        Add To Compare
                      </label>
                    </div>
                    <td className="bba_dm_center bba_dm_clg-fees">
                      <p>{college.fees || "--"}</p>
                      <a href={college.applyLink} className="bba_dm_action-link-2 bba_dm_apply">
                        &#8644; Compare Fees
                      </a>
                      <p className="bba_dm_grey-subtitle">BAA</p>
                      <p className="bba_dm_grey-subtitle">First Year</p>
                    </td>
                    <td className="bba_dm_center">
                      <p className="bba_dm_clg-fees">{college.averagePackage || "--"}</p>
                      <p className="bba_dm_grey-subtitle">Average Package</p>
                      <p className="bba_dm_clg-fees">{college.highestPackage || "--"}</p>
                      <p className="bba_dm_grey-subtitle">Highest Package</p>
                      <a href={college.applyLink} className="bba_dm_action-link-2 bba_dm_apply">
                        &#8644; Compare Placement
                      </a>
                    </td>
              ...
              {tableIndex === 1 && (
                <div className="bba_dm_horizontal-scroll-container">
                <h3>College Application Forms</h3>
                <div className="bba_dm_horizontal-scroll" ref={scrollRef}>
                  {collegeScrolls.map((college) => (
                    <div className="bba_dm_college-hori-card" key={college.id}>
                      <div>
                        <img src={college.image} alt={college.name} className="" />
                        <div className="bba_dm_rating">⭐ {college.rating}</div>
                      </div>
                      <div className="bba_dm_college-details">
                        <span className="bba_dm_scroll-college-name">{college.name}</span>
                        <span className="bba_dm_scroll-college-location">{college.location}</span>
                        <span className="bba_dm_college-category">{college.category}</span>
                        <span className="bba_dm_college-description">{college.description}</span>
                        <span className="bba_dm_college-package">{college.highestPackage}</span>
                        <a href={college.applyLink} className="bba_dm_hori-action-link bba_dm_hori-apply">
                          &rarr; Apply Now
                        </a>
                      </div>
                    </div>
              ...
              {tableIndex === 2 && (
                <div className="bba_dm_recommendation-container">
                <h3>How likely would you recommend this website to your friend?</h3>
                <div className="bba_dm_recommendation-scale">
                  {[...Array(10).keys()].map((num) => (
                    <span
                      key={num}
                      className={`bba_dm_number ${hoveredNumber >= num + 1 ? 'bba_dm_hovered' : ''}`}
              ...
              {tableIndex ===3 && (
                   <div className="bba_dm_collections-container">
                   <h2 className="bba_dm_collections-title">Top Collections for BBA/BMS</h2>
                   <div className="bba_dm_collections-scroll">
                     {collections.map((collection, index) => (
                       <div key={index} className="bba_dm_collection-card">
                         <div className="bba_dm_card-image-container">
                           <img
                             src={collection.imageUrl}
                             alt={collection.title}
                             className="bba_dm_card-image"
                           />
                           <div className="bba_dm_gradient-overlay"></div>
                           <div className="bba_dm_card-content">
                             <h3 className="bba_dm_card-title">{collection.title}</h3>
                             <div className="bba_dm_logo-container">
                               <div className="bba_dm_logo-stack">
                                 {collection.logos.map((logo, idx) => (
                                   <img
                                     key={idx}
                                     src={logo}
                                     alt="College logo"
                                     className="bba_dm_collection-logo"
                                   />
                                 ))}
                               </div>
                               <div className="bba_dm_more-count">+{collection.colleges} More</div>
                             </div>
                           </div>
                         </div>
                       </div>
